...
namespace Mirror {
    ...
    public class NetworkManagerHUDEnhanced : MonoBehaviour {
        ...
        private string serverPortInput = "7777";
        ...
        void StartButtons() {
            if (!NetworkClient.active) {
                ...
                GUILayout.EndHorizontal();                // Server Only
#if UNITY_WEBGL
                // cant be a server in webgl build
                ...
                if (GUILayout.Button("Server Only")) {
                    if (ushort.TryParse(serverPortInput, out ushort port)) {
                        if (Transport.active is PortTransport serverPortTransport) {
                            serverPortTransport.Port = port;
                        }
                    }
                    manager.StartServer();
                }
